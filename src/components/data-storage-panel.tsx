...
type StorageTabId = "file" | "appearance";
...
export function DataStoragePanel({
  ...
  const [activeTab, setActiveTab] = useState<StorageTabId>("file");
  ...
  const layer = (
    ...
    >
      <aside
        className="flex h-full w-full max-w-md flex-col border-l border-[var(--border)] bg-[var(--panel-solid)] text-[var(--text)] shadow-2xl"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header className="flex shrink-0 items-start justify-between gap-3 border-b border-[var(--border)] px-5 py-4">
          <div>
            <h2 id="data-storage-panel-title" className="text-base font-semibold text-[var(--text)]">
              Daten &amp; Speicher
            </h2>
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1.5 text-[var(--muted)] hover:bg-[var(--control-hover)] hover:text-[var(--text)]"
        ...
        <div className="flex shrink-0 gap-1 border-b border-[var(--border)] px-3 py-2">
          <button
            ...
            className={[
              ...
              activeTab === "file" ? "dock-control-active" : "dock-control",
            ].join(" ")}
          >
          ...
          <button
            ...
            className={[
              "inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium",
              activeTab === "appearance" ? "dock-control-active" : "dock-control",
            ].join(" ")}
          >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4 space-y-8">
          ...
          <section className="border-t border-[var(--border)] pt-6">
            <p className="text-xs text-[var(--muted)]">{formatAppVersionLabel()}</p>
          </section>
        </div>
