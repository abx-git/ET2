...
Beispiel: `https://abx-git.github.io/ET2/`
...
3. In `render.yaml` **vor dem Deploy** anpassen:
   ```yaml
   ET2_VAULT_CORS_ORIGINS: https://<IHR-GITHUB-USER>.github.io,http://localhost:3000
   ```
...
|----------|----------|---------|
| `NEXT_PUBLIC_ET2_VAULT_API_URL` | `https://t2-vault-xxxx.onrender.com` | ja (für Server-Option) |
| `NEXT_PUBLIC_BASE_PATH` | `/ET2` | nein (Standard: `/` + Repo-Name) |
...
| Variable | Wirkung |
|----------|---------|
| `NEXT_PUBLIC_BASE_PATH` | URL-Pfad der App (`/ET2` bei Project Pages). Leer lassen → automatisch `/<repo-name>`. |
| `NEXT_PUBLIC_ET2_VAULT_API_URL` | Vault-API für den statischen Build. Ohne Wert: keine Server-Option. |
...
| Server-Option fehlt | `NEXT_PUBLIC_ET2_VAULT_API_URL` setzen und neu deployen |
| Vault-Verbindung schlägt fehl | `ET2_VAULT_CORS_ORIGINS` auf dem Vault-Host muss `https://<user>.github.io` enthalten |
